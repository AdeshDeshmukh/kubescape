...
func TestLoadResourcesFromFiles_SurfacesTypoKindInKnownGroup(t *testing.T) {
	dir := t.TempDir()
	writeManifestFixture(t, dir, "valid.yaml", validPodManifest)
	typo := writeManifestFixture(t, dir, "typo.yaml", `apiVersion: apps/v1
kind: Deplyment
metadata:
  name: test
  namespace: default
`)
	...
	require.Contains(t, workloads, filepath.Join(dir, "valid.yaml"))
	require.Len(t, skips, 1)
	assert.Equal(t, typo, skips[0].Path)
	assert.Contains(t, skips[0].Reason, "not a valid Kubernetes kind")
}
